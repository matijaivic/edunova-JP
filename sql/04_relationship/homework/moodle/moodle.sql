# c:\xampp\mysql\bin\mysql -uedunova -pedunova < D:\Edunova\JP\04_exercise\Moodle\moodle.sql
...
alter table assign_user_mapping add foreign key (userid) references moodle_users.user(id);
...
alter table assignfeedback_editpdf_quick add foreign key (userid) references moodle_users.user(id);
...
alter table assign_user_flags add foreign key (userid) references moodle_users.user(id);
